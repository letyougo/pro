# Generated by Django 2.0 on 2017-12-28 11:43

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    replaces = [('exam', '0001_initial'), ('exam', '0002_auto_20171204_1723'), ('exam', '0003_auto_20171204_1724'), ('exam', '0004_auto_20171205_1036'), ('exam', '0005_auto_20171205_1054'), ('exam', '0006_auto_20171205_1056'), ('exam', '0007_auto_20171205_1125'), ('exam', '0008_auto_20171207_1050'), ('exam', '0009_auto_20171214_1006'), ('exam', '0010_auto_20171223_2023'), ('exam', '0011_auto_20171223_2127')]

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Center',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('user', models.OneToOneField(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='注册用户')),
                ('center_name', models.CharField(blank=True, max_length=128, null=True, verbose_name='中心名字')),
                ('create_time', models.DateTimeField(auto_now_add=True, null=True, verbose_name='创建时间')),
                ('update_time', models.DateTimeField(auto_now=True, null=True, verbose_name='更新时间')),
            ],
            options={
                'verbose_name': '考试中心',
                'verbose_name_plural': '考试中心',
            },
        ),
        migrations.CreateModel(
            name='Office',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('office_name', models.CharField(blank=True, max_length=128, null=True)),
                ('exam_name', models.CharField(blank=True, max_length=128, null=True)),
                ('user', models.OneToOneField(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': '考试办',
                'verbose_name_plural': '考试办',
            },
        ),
        migrations.CreateModel(
            name='School',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(blank=True, max_length=128, null=True)),
                ('user', models.OneToOneField(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': '学校端',
                'verbose_name_plural': '学校端',
            },
        ),
        migrations.CreateModel(
            name='Teacher',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(blank=True, max_length=128, null=True)),
                ('phone', models.CharField(blank=True, max_length=128, null=True)),
                ('idcard', models.CharField(blank=True, max_length=256, null=True)),
                ('bankcard', models.CharField(blank=True, max_length=256, null=True)),
                ('bankinfo', models.TextField()),
                ('school', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.School')),
            ],
            options={
                'verbose_name': '教师列表',
                'verbose_name_plural': '教师列表',
            },
        ),
        migrations.CreateModel(
            name='Exam',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('time', models.DateField(auto_now=True, null=True)),
                ('total', models.CharField(blank=True, max_length=128, null=True)),
                ('desc', models.TextField()),
                ('office', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.Office')),
            ],
            options={
                'verbose_name': '考试信息',
                'verbose_name_plural': '考试信息',
            },
        ),
        migrations.CreateModel(
            name='SchoolExam',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('total', models.IntegerField(blank=True, max_length=256, null=True)),
                ('exam', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.Exam')),
                ('school', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.School')),
            ],
        ),
        migrations.CreateModel(
            name='TeacherExam',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('total', models.IntegerField(blank=True, max_length=256, null=True, verbose_name='老师应得监考费')),
                ('schoolexam', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.Schoolexam', verbose_name='在某学校的考试')),
                ('teacher', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.Teacher', verbose_name='监考老师')),
                ('create_time', models.DateTimeField(auto_now_add=True, null=True, verbose_name='创建时间')),
                ('update_time', models.DateTimeField(auto_now=True, null=True, verbose_name='更新时间')),
            ],
            options={
                'verbose_name': '学校分配给老师的考试',
                'verbose_name_plural': '学校分配给老师的考试',
            },
        ),
        migrations.AlterModelOptions(
            name='schoolexam',
            options={'verbose_name': '考试办分配给学校的考试', 'verbose_name_plural': '考试办分配给学校的考试'},
        ),
        migrations.AlterField(
            model_name='exam',
            name='time',
            field=models.DateField(blank=True, null=True),
        ),
        migrations.AddField(
            model_name='exam',
            name='create_time',
            field=models.DateTimeField(auto_now_add=True, null=True, verbose_name='创建时间'),
        ),
        migrations.AddField(
            model_name='exam',
            name='update_time',
            field=models.DateTimeField(auto_now=True, null=True, verbose_name='更新时间'),
        ),
        migrations.AddField(
            model_name='office',
            name='create_time',
            field=models.DateTimeField(auto_now_add=True, null=True, verbose_name='创建时间'),
        ),
        migrations.AddField(
            model_name='office',
            name='update_time',
            field=models.DateTimeField(auto_now=True, null=True, verbose_name='更新时间'),
        ),
        migrations.AddField(
            model_name='school',
            name='create_time',
            field=models.DateTimeField(auto_now_add=True, null=True, verbose_name='创建时间'),
        ),
        migrations.AddField(
            model_name='school',
            name='update_time',
            field=models.DateTimeField(auto_now=True, null=True, verbose_name='更新时间'),
        ),
        migrations.AddField(
            model_name='schoolexam',
            name='create_time',
            field=models.DateTimeField(auto_now_add=True, null=True, verbose_name='创建时间'),
        ),
        migrations.AddField(
            model_name='schoolexam',
            name='update_time',
            field=models.DateTimeField(auto_now=True, null=True, verbose_name='更新时间'),
        ),
        migrations.AddField(
            model_name='teacher',
            name='create_time',
            field=models.DateTimeField(auto_now_add=True, null=True),
        ),
        migrations.AddField(
            model_name='teacher',
            name='update_time',
            field=models.DateTimeField(auto_now=True, null=True),
        ),
        migrations.AlterModelOptions(
            name='teacher',
            options={'ordering': ['-create_time', '-update_time'], 'verbose_name': '教师列表', 'verbose_name_plural': '教师列表'},
        ),
        migrations.AlterField(
            model_name='exam',
            name='desc',
            field=models.TextField(verbose_name='考试描述'),
        ),
        migrations.AlterField(
            model_name='exam',
            name='office',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.Office', verbose_name='所属考试办'),
        ),
        migrations.AlterField(
            model_name='exam',
            name='time',
            field=models.DateField(blank=True, null=True, verbose_name='考试时间'),
        ),
        migrations.AlterField(
            model_name='exam',
            name='total',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='总费用'),
        ),
        migrations.AlterField(
            model_name='office',
            name='exam_name',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='考试名字'),
        ),
        migrations.AlterField(
            model_name='office',
            name='office_name',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='考试办名字'),
        ),
        migrations.AlterField(
            model_name='office',
            name='user',
            field=models.OneToOneField(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='注册用户'),
        ),
        migrations.AlterField(
            model_name='school',
            name='name',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='学校名字'),
        ),
        migrations.AlterField(
            model_name='school',
            name='user',
            field=models.OneToOneField(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='注册用户'),
        ),
        migrations.AlterField(
            model_name='schoolexam',
            name='exam',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.Exam', verbose_name='所属考试'),
        ),
        migrations.AlterField(
            model_name='schoolexam',
            name='school',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.School', verbose_name='指派学校'),
        ),
        migrations.AlterField(
            model_name='schoolexam',
            name='total',
            field=models.IntegerField(blank=True, max_length=256, null=True, verbose_name='学校总监考费'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='bankcard',
            field=models.CharField(blank=True, max_length=256, null=True, verbose_name='银行卡'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='bankinfo',
            field=models.TextField(verbose_name='银行信息'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='create_time',
            field=models.DateTimeField(auto_now_add=True, null=True, verbose_name='创建时间'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='idcard',
            field=models.CharField(blank=True, max_length=128, null=True, unique=True, verbose_name='身份证'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='name',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='老师名字'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='phone',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='手机号码'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='school',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.School', verbose_name='所在学校'),
        ),
        migrations.AlterField(
            model_name='teacher',
            name='update_time',
            field=models.DateTimeField(auto_now=True, null=True, verbose_name='更新时间'),
        ),
        migrations.AddField(
            model_name='office',
            name='admin_name',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='考办管理员'),
        ),
        migrations.AddField(
            model_name='office',
            name='admin_phone',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='考办联系电话'),
        ),
        migrations.AddField(
            model_name='school',
            name='admin_name',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='学校管理员'),
        ),
        migrations.AddField(
            model_name='school',
            name='admin_phone',
            field=models.CharField(blank=True, max_length=128, null=True, verbose_name='学校联系电话'),
        ),
        migrations.AlterField(
            model_name='schoolexam',
            name='school',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='exam.School', verbose_name='监考学校'),
        ),
        migrations.AddField(
            model_name='exam',
            name='status',
            field=models.CharField(blank=True, choices=[(0, '通过'), (1, '审核中'), (2, '未审核')], max_length=128, null=True, verbose_name='状态'),
        ),
        migrations.AddField(
            model_name='schoolexam',
            name='status',
            field=models.CharField(blank=True, choices=[(0, '通过'), (1, '审核中'), (2, '未审核')], max_length=128, null=True, verbose_name='状态'),
        ),
    ]
