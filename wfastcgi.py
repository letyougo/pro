...
def start_file_watcher(path, restart_regex):
    ...
    log('wfastcgi2.py will restart when files in %s are changed: %s' % (path, restart_regex))
    def watcher(path, restart):
        for filename in enum_changes(path):
            if not filename:
                log('wfastcgi2.py exiting because the buffer was full')
                run_exit_tasks()
                ExitProcess(0)
            elif restart.match(filename):
                log('wfastcgi2.py exiting because %s has changed, matching %s' % (filename, restart_regex))
...
def main():
    initialized = False
    log('wfastcgi2.py %s started' % __version__)
    ...
    try:
        ...
        while True:
            ...
            with handle_response(fcgi_stream, record, output.getvalue, errors.getvalue) as response:
                if not initialized:
                    log('wfastcgi2.py %s initializing' % __version__)
                    ...
                    log('wfastcgi2.py %s initialized' % __version__)
                    initialized = True
        ...
        maybe_log('Unhandled exception in wfastcgi2.py: ' + traceback.format_exc())
    except BaseException:
        maybe_log('Unhandled exception in wfastcgi2.py: ' + traceback.format_exc())
        raise
    finally:
        run_exit_tasks()
        maybe_log('wfastcgi2.py %s closed' % __version__)
