<template>
  <div class="exam-teacher">
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="{path:'/teacher_list?id='+exam_item.school_id}">学校管理({{exam_item.school_name}})
      </el-breadcrumb-item>
    ...
    <el-form :inline="true" class="demo-form-inline" :loading="loading" size="small">
      ...
      <el-form-item>
        <el-button-group>
          ...
          <el-button @click="upload_form.visible=true">
            批量上传
          </el-button>
    ...
    <el-dialog title="批量上传" :visible.sync="upload_form.visible">
      <el-form :inline="true">
        <el-form-item>
      ...
      <el-table :data="upload_form.list">
        ...
        <el-table-column>
          <template scope="scope">
            <el-button @click=" action(scope.$index) ">重试</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      action(i){
        ...
        let
            that = this ,
            list = that.upload_form.list,
            res =  request.post('/api/teacherexam/', {
              school_id:that.exam_item.school_id ,
              name:list[i].name,
              total:list[i].total,
              school_exam:that.exam_item.id
            })
            .then(item=>{
               that.upload_form.list[i].status = '成功'
            }).catch(error=>{
             that.upload_form.list[i].status = '失败'
          })
      },
      async upload() {
        let
          list = this.upload_form.list,
          that = this
      ...
      onchange(f) {
        ...
        reader.onload = function (e) {
          ...
          list = list.map(obj => {
            obj.name = obj['老师姓名']
            obj.total = parseInt(obj['监考费'])
            obj.status = '开始'
            delete obj['老师姓名']
            delete obj['监考费']
            return obj
          });
          that.upload_form.list = list
        };
        reader.readAsBinaryString(file);
      },
      downloadTemplate() {
        location.href = '/schooltemplate'
      },
      get_teacher(id) {
      ...
      async fetch() {
        ...
        let page_size = this.page_size,
          ...
          response = await request.get('/api/teacherexam/?schoolexam_id=' + id, {
          params: {
            page_size,
            page_num,
            search
